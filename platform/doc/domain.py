...
from platform.doc.extract import MAX_TEXT_CHARS as _MAX_TEXT_CHARS
from platform.doc.extract import sanitize_pg_text
from platform.doc.jobs import register as register_extract_jobs
from platform.doc.storage import BlobStore, get_blob_store
...
def get_asset_preview(
    ...
    *,
    ...
    from platform.doc.extract import preview_excel
...
def apply_extract_job(session: Session, payload: dict[str, Any]) -> None:
    ...
    try:
        if not row.name:
            raise AppError("not_found", "asset has no stored blob")
        data = _store_for_row(row).get(row.name)
        from platform.doc.extract import extract_bytes
